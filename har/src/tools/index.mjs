 ...
 * The tool surface.
 *
 * Four tools. `exec` is the capability gm prescribes: the agent runs real code
 * and reads real output. Each tool follows pi's `AgentTool` contract -- it
 * throws on failure rather than encoding errors in `content`, and the harness
 * converts that throw into a witnessed error result.
 */
 ...
 * separately. Resolves with the observed outcome rather than rejecting on a
 * non-zero exit: a failing command is a real result the agent must read.
 */
  ...
  return new Promise((resolvePromise) => {
    ...
    const timer = setTimeout(() => {
      ...
      try {
        ...
        // Process already gone; the close handler still settles the promise.
      }
    }, timeoutMs);
...
/**
 * Build the tool set bound to a workspace root and configuration.
 * `redact` is applied to everything the model or the log can see.
 */
export function createTools({ root, config, redact = (s) => s }) {
  ...
  const exec = {
    ...
    async execute(toolCallId, params, signal) {
      ...
      return textResult(redact(parts.join("\n\n")), {
        ...
        cwd,
        // A command that produced no output and exited zero changed nothing
        // observable; the L3 classifier uses this to detect flat motion.
  ...
  const writeFile = {
    ...
    async execute(toolCallId, params) {
      ...
      try {
        ...
        // New file; there is no previous content to compare against.
      }
  ...
  return [exec, readFile, writeFile, listDir];
}
